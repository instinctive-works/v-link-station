// LiveLink (MotionBuilder) node plugin — stub (未実装)
window.NodePlugins['livelink-mb'] = {
  label:       'LiveLink MB in',
  icon:        '🕺',
  ...
  create(pos) {
    ...
    const name   = window.nextUniqueName('livelink-mb', 'LiveLink MB');
    window.createPluginNode('livelink-mb', nodeId, pos);
  ...
  mount(nodeId, nodeEl) {
    nodeEl.innerHTML = `
      <div class="node-header node-livelink" id="nheader-${nodeId}">
        ...
        <input class="node-name" id="ename-${nodeId}" value="LiveLink MB" />
